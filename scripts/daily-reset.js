  ...
  try {
    ...
    for (const userCredit of usersToReset) {
      try {
        ...
        console.error(`Error resetting credits for user ${userCredit.userId}:`, error);
        errorCount++;
      }
    }

    console.log(`✅ Daily reset completed: ${resetCount} users reset, ${errorCount} errors`);
  ...
  try {
    ...
    for (const userCredit of subscribedUsers) {
      try {
        ...
        console.error(`Error adding bonus credits for user ${userCredit.userId}:`, error);
      }
    }

    console.log(`✅ Bonus credits added: ${bonusCount} users received ${totalBonusCredits} total credits`);
...
async function main() {
  ...
  try {
    ...
    console.log(`Reset: ${resetResult.success ? '✅' : '❌'} ${resetResult.resetCount || 0} users`);
    console.log(`Bonus: ${bonusResult.success ? '✅' : '❌'} ${bonusResult.totalBonusCredits || 0} credits added`);
