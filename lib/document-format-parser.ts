...
export class DocumentFormatParser {
   ...
   */
  parseMarkdown(content: string, metadata?: Partial<DocumentMetadata>): ParsedDocument {
  ...
  parseJSON(jsonContent: string): ParsedDocument {
    try {
      ...
      throw new Error(`Failed to parse JSON document: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  parsePlainText(content: string, metadata?: Partial<DocumentMetadata>): ParsedDocument {
      ...
      switch (element.type) {
        ...
        case 'list':
          element.items.forEach((item, index) => {
            const bullet = element.listType === 'ordered' ? `${index + 1}.` : '-';
